...
class InitializeData:
    ...
    def __init__(self, image_path, coarse_label_path, fine_label_path, ratio_choose=1,
                 ratio_divide=0.9):
        self.all_items = []
        ...
        for i in range(len(image_list)):
            assert image_list[i] == label_list[
                i], "image and label should be same : {} and {}".format(image_list[i],
                                                                        label_list[i])
        ...
        print("target size: {}\r\ncontrast size: {}".format(len(self.targets),
                                                            len(self.contrasts)))
...
class SegDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        image = np.array(
            imread(os.path.join(self.init_data.image_path, self.items[index]), pilmode="RGB"))
        label_coarse = np.expand_dims(
            imread(os.path.join(self.init_data.coarse_label_path, self.items[index]),
                   pilmode="L"), -1)
        label_fine = np.expand_dims(
            imread(os.path.join(self.init_data.fine_label_path, self.items[index]),
                   pilmode="L"), -1)

        sample = {'file_name': self.items[index],
                  'image': image,
...
class InitializeDataGeneral:
    ...
    def __init__(self, image_path, coarse_label_path, fine_label_path, generate_label_path,
                 ratio_choose=1, ratio_divide=0.9):
        self.all_items = []
...
class SegDatasetGeneral(Dataset):
    ...
    def __getitem__(self, index):
        ...
        label_generate = np.expand_dims(
            imread(os.path.join(self.init_data.generate_label_path, self.items[index]),
                   pilmode="L"), -1)
