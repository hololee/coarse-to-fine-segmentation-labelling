...
class UNet(nn.Module):
    ...
    def __init__(self):
        ...
        self.level5_upsample = nn.ConvTranspose2d(in_channels=1024, out_channels=512, kernel_size=4, stride=2,
                                                  padding=1)
        ...
        self.level4_upsample = nn.ConvTranspose2d(in_channels=512, out_channels=256, kernel_size=4, stride=2, padding=1)
        ...
        self.level3_upsample = nn.ConvTranspose2d(in_channels=256, out_channels=128, kernel_size=4, stride=2, padding=1)
        ...
        self.level2_upsample = nn.ConvTranspose2d(in_channels=128, out_channels=64, kernel_size=4, stride=2, padding=1)
        ...
        self.level1_conv5 = nn.Conv2d(64, 1, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, x):
        self.feature1 = F.relu(self.level1_bat2(self.level1_conv2(F.relu(self.level1_bat1(self.level1_conv1(x))))))
        self.feature2 = F.relu(self.level2_bat2(
            self.level2_conv2(F.relu(self.level2_bat1(self.level2_conv1(self.level1_maxpool(self.feature1)))))))
        self.feature3 = F.relu(self.level3_bat2(
            self.level3_conv2(F.relu(self.level3_bat1(self.level3_conv1(self.level2_maxpool(self.feature2)))))))
        self.feature4 = F.relu(self.level4_bat2(
            self.level4_conv2(F.relu(self.level4_bat1(self.level4_conv1(self.level3_maxpool(self.feature3)))))))

        self.feature5 = self.level5_bat2(
            self.level5_conv2(self.level5_bat1(self.level5_conv1(self.level4_maxpool(self.feature4)))))
        self.feature4_up = torch.cat((self.level5_upsample(self.feature5), self.feature4), dim=1)
        self.feature4_2 = F.relu(
            self.level4_bat4(self.level4_conv4(F.relu(self.level4_bat3(self.level4_conv3(self.feature4_up))))))
        self.feature3_up = torch.cat((self.level4_upsample(self.feature4_2), self.feature3), dim=1)
        self.feature3_2 = F.relu(
            self.level3_bat4(self.level3_conv4(F.relu(self.level3_bat3(self.level3_conv3(self.feature3_up))))))
        self.feature2_up = torch.cat((self.level3_upsample(self.feature3_2), self.feature2), dim=1)
        self.feature2_2 = F.relu(
            self.level2_bat4(self.level2_conv4(F.relu(self.level2_bat3(self.level2_conv3(self.feature2_up))))))
        self.feature1_up = torch.cat((self.level2_upsample(self.feature2_2), self.feature1), dim=1)
        self.feature1_2 = F.relu(
            self.level1_bat4(self.level1_conv4(F.relu(self.level1_bat3(self.level1_conv3(self.feature1_up))))))
